...
class MyAgent(spade.Agent.Agent):
    ...
    def enviarMensaje(self):
		...
		self.msgS.setOntology("demo3")
                ...
                self.msgS.setContent("Movimiento1ON")
                self.send(self.msgS)
		...
		self.msgS.setOntology("demo3")
                self.msgS.addReceiver(receiver)
                self.msgS.setContent("Movimiento1OFF")
                self.send(self.msgS)
    ...
    class recibeSubscribe(spade.Behaviour.EventBehaviour):
      	def _process(self):
        	...
        	if self.msgR:
			print "Suscripcion de movimiento recibida"
